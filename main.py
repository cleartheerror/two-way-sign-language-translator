...
classifier = load_model(r'd:\project1\two-way-sign-language-translator\model.h5')
def give_char():
       import numpy as np
...
# ...existing code...
op_dest=r"d:\project1\two-way-sign-language-translator\filtered_data" + "\\"
alpha_dest=r"d:\project1\two-way-sign-language-translator\alphabet" + "\\"
# ...existing code...
...
def func(a):
       ...
       for i in words:
              ...
              if(flag==-1):
                     for j in i:
                            print(j)
                            # the files in `alphabet/` use a .gif extension but are actually
                            # JPEG still‑images (see repository). PIL returns a JpegImageFile,
                            # so there is no n_frames property; our fallback logic below handles
                            # this by assuming a single frame.
                            im = PIL.Image.open(alpha_dest+str(j).lower()+"_small.gif")
                            # some of the alphabet files may not be animated GIFs (e.g. JPEGs mis‑named),
                            # so fall back to a single frame when "n_frames" is missing
                            frameCnt = getattr(im, "n_frames", 1)
                            for frame_cnt in range(frameCnt):
                                   try:
                                          im.seek(frame_cnt)
                                   except EOFError:
                                          # in case the image has fewer frames than claimed
                                          break
                                   im.save("tmp.png")
...
class StartPage(tk.Frame):
       ...
       def __init__(self, parent, controller):
              ...
              button2.pack()
              image_path = os.path.join(os.path.dirname(__file__), "Two Way Sign Language Translator.png")
